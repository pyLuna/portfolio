...
export async function GET() {
  ...
  if (!basicInfo)
    return error(
      ...
      { status: 404, string_code: ErrorCode.basic_info_not_found }
    );
...
export async function POST(req: NextRequest) {
  ...
  if (!isValid) {
    ...
    return error(
      ...
      { status: 401, string_code: ErrorCode.token_expired }
    );
  }
  ...
  if (existing)
    return error(
      ...
      { status: 400, string_code: ErrorCode.basic_info_already_exists }
    );
...
export async function PATCH(req: NextRequest) {
  ...
  await updateBasicInfo({ ...data, admin_id: id });
